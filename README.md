...
> **当前发布版本**：**2.0.62**（稳定版）  
> **版本发布日期**：**2026-08-22**（与根目录 `**version-info.json`** 中的 `version`、`release_date` 保持一致；后续迭代以此文件为准。）  
...
- `**burst_keep_ratio**`：每组保留比例，相对**本组总张数**（如 `0.1` 约保留 10%）。快速模式也按全组计算，不会先抽 1/3 再乘比例。  
