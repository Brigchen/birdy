...
2. **鸟体检测与分割**：批量定位鸟体目标，支持置信度/面积过滤，为后续裁剪、分类与归档提供结构化结果。按 **1280** 推理；仅当长边 **>5000** 时再对画面中心检一次，降低远距小鸟漏检。
3. **物种自动识别与分级归档**：支持本地模型与豆包视觉 API 两套识别路径，按目/科/属/种自动整理输出目录；GUI 物种识别栏可指定文件夹并**单独裁切识别**，不必加入主流程。
4. **地理信息写入与反查**：支持 GPS EXIF 批量写入与地名编码，为归档检索和水印文案提供位置数据。
5. **水印图批量生成与报告输出**：可批量生成带时间/地点/物种等信息的水印成片，并输出处理报告，便于分享与追溯；含 **视频裁剪与稳定**（vidstab + ffmpeg 保留音轨，见 v2.0.11）。水印保存为 **接近无损**（JPEG quality=100 / 4:4:4，或 PNG）。
6. **连拍动图 / 视频**：GUI「动图生成」可将连拍序列导出为 **动画 WebP** 或 **MP4**（白平衡、按裁剪框自动曝光、定点/跟踪裁剪与叠水印）；**定点（三脚架）** 或 **跟踪** 两种模式，后续帧用 **鸟体检测 + 卡尔曼** 自动传播标定点（默认 2 张/秒）。图片列表与定位写入相片目录下的 **`.birdy-burst.json` 项目文件**，关闭后再打开可恢复。
...
11. **镜头 MTF 比较（独立工具）**：`tools/MTF-meter/` 单独 GUI，ISO 12233 测 MTF50/30/10、过锐峰值与中心/边比并按镜头/焦距分组，或对同场景多镜头做相对比较（不接入主界面）。
12. **清晰度打分（独立工具）**：`tools/clarity-meter/` 调用认种前模糊评分，生成 HTML 表格报告，**不删除**源图。

提供 **PyQt5 图形界面** 与 **命令行** 两种使用方式，既可交互式操作，也可用于脚本化批处理。

> **当前发布版本**：**2.1.5**（稳定版）  
> **版本发布日期**：**2026-09-20**（与根目录 `**version-info.json`** 中的 `version`、`release_date` 保持一致；后续迭代以此文件为准。）  
...
| 模块            | 说明                                                                                                           |
| ------------- | ------------------------------------------------------------------------------------------------------------ |
| **鸟体检测**      | YOLOv8 分割，`imgsz=1280`；长边 >5000 时中心复检；支持置信度与面积过滤、可选鸟眼辅助                                                                                  |
...
| **连拍动图/视频** | GUI「动图生成」：**WebP** 或 **MP4**；**定点（三脚架）** / **跟踪**；首图标定点+裁剪区后自动传播；按裁剪框**自动曝光**（强度可调，当前图即时预览）；帧率默认 2 张/秒；选项参数下次打开自动恢复 |
...
| [MTF-meter](tools/MTF-meter/) | ISO 12233 MTF / 同场景镜头比较 | `MTF.bat`；独立窗口，不接入 Birdy 主界面 |
...
- `**burst_keep_ratio**`：每组保留比例，相对**本组总张数**（如 `0.1` 约保留 10%）。快速模式先等步长抽约全组一半作候选，再按此比例精选。  
